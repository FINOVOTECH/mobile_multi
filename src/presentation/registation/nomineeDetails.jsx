...
const NomineeDetails = ({ setStatus, isLoading, setIsLoading, pinVerify }) => {
  ...
  const isNomineeValid = (nominee) => {
    ...
    const addressValid = nominee.addressSameAsUser
      ? true
  ...
  const handleSubmitNomination = async () => {
    const invalidNominees = nominees.filter(
      (nominee) => !isNomineeValid(nominee)
    );
    if (invalidNominees.length > 0) {
      Alert.alert(
        "Validation Error",
        "Please fill all nominee details correctly"
      );
      return;
    }
    ...
    console.log("paylaodd nominee ::", payload, pinVerify);
    try {
      ...
      if (response.ok && data.status === "SUCCESS") {
        ...
        const errorMessage =
          data.message || data.error || "Nomination submission failed";
    ...
    <View key={index} style={styles.nomineeContainer}>
            ...
            <View style={styles.horizontalOptions}>
              {relationshipOptions.map((option) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.optionButton,
                    nominee.nomineeRelationship === option &&
                      styles.optionButtonSelected,
                  ]}
                  onPress={() =>
                    updateNominee(index, "nomineeRelationship", option)
                  }
                >
                  <Text
                    style={[
      ...
      <View style={styles.inputContainer}>
        ...
        <TextInput
          ...
          onChangeText={(text) =>
            updateNominee(index, "percentage", parseInt(text) || 0)
          }
          keyboardType="numeric"
          placeholderTextColor="#999"
          editable={false}
            ...
            <View style={styles.horizontalOptions}>
              {identityProofOptions.map((option) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.optionButton,
                    nominee.IdentityProofType === option &&
                      styles.optionButtonSelected,
                  ]}
                  onPress={() =>
                    updateNominee(index, "IdentityProofType", option)
                  }
                >
                  <Text
                    style={[
      ...
      <View style={styles.inputContainer}>
        ...
        <TextInput
          ...
          onChangeText={(text) =>
            updateNominee(index, "IdentityProofNumber", text)
          }
          placeholderTextColor="#999"
        />
      </View>
          ...
          <View>
            ...
            <View style={styles.titleContainer}>
              ...
              <Text style={styles.subtitleText}>
                Please provide nominee information
              </Text>
            </View>

            <View style={styles.formContainer}>
              {nominees.map((nominee, index) =>
                renderNomineeForm(nominee, index)
              )}
              ...
              {nominees.length < 2 && (
                <TouchableOpacity
                  ...
                  onPress={addNominee}
                >
                  <Text style={styles.addNomineeButtonText}>
                    + Add Another Nominee
                  </Text>
                </TouchableOpacity>
              )}
